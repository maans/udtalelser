...
function resolveTeacherMatch(raw) {
  ...
  // No hardcoded teacher directory in v1.0.
  // We keep "resolve" very lightweight: if the user typed a name, we keep it;
  // initials will be derived by toInitials() when needed.
...
function teacherInitialFromStudent(st, which) {
  if (!st) return '';
  const rawName = (which === 2 ? (st.kontaktlaerer2 || '') : (st.kontaktlaerer1 || '')).toString().trim();
  const rawIni  = (which === 2 ? (st.kontaktlaerer2_initialer || '') : (st.kontaktlaerer1_initialer || '')).toString().trim();
  if (rawIni) return rawIni.toUpperCase();
  return toInitials(rawName);
}

function groupKeyFromStudent(st) {
  const a = teacherInitialFromStudent(st, 1);
  const b = teacherInitialFromStudent(st, 2);
...
function updateTeacherDatalist() {
  // Identity picker (custom dropdown)
  ...
  const initials = Array.from(new Set(
    (studs || []).flatMap(st => [teacherInitialFromStudent(st,1), teacherInitialFromStudent(st,2)])
      .filter(Boolean)
      .map(x => x.toUpperCase().trim())
  )).sort((a,b)=>a.localeCompare(b,'da'));

  // If no students yet, disable the picker.
  if (!initials.length){
    input.value = '';
    input.disabled = true;
    btn.disabled = true;
    ...
    if (!input.placeholder) input.placeholder = 'Initialer';
  }

  const allItems = initials.map(code => ({ kind: 'ini', code, name: '' }));
...
function initMarksSearchPicker(){
  ...
  function renderMenu(){
    ...
    if (!items.length){
      ...
      empty.textContent = 'Ingen match – skriv fx initialer…';
      menu.appendChild(empty);
      return;
    }
    for (const it of items){
      ...
      if (it.kind === 'alias' || it.kind === 'ini'){
        left.textContent = it.code;
        right.textContent = (it.kind === 'alias' && it.name) ? `(${it.name})` : '';
      ...
      if (e.key === 'Enter') {
        ...
        if (!menu.hidden && activeIndex >= 0) {
          ...
          if (el && el._item) {
            ...
            const it = el._item;
            closeMenu();
            const rawName = (it.kind === 'alias' || it.kind === 'ini') ? it.code : it.name;
...
"FORSTANDER_NAVN": settings.forstanderName || '',
  ...
  const STUDENT_COLMAP = {
    ...
    kontakt1_ini: new Set(["initialer fork-lærer1","initialer fork-laerer1","initialer for k-lærer1","initialer for k-laerer1","kontaktlærer1 initialer","kontaktlaerer1 initialer","k-lærer1 initialer","k-laerer1 initialer","kontakt1 initialer"]),
    kontakt2_ini: new Set(["initialer fork-lærer2","initialer fork-laerer2","initialer for k-lærer2","initialer for k-laerer2","kontaktlærer2 initialer","kontaktlaerer2 initialer","k-lærer2 initialer","k-laerer2 initialer","kontakt2 initialer"])
  };
  ...
  function normalizeStudentRow(row, map) {
    ...
    const k1_ini = (get('kontakt1_ini') || '').trim().toUpperCase();
    const k2_ini = (get('kontakt2_ini') || '').trim().toUpperCase();
    return {
      fornavn,
      efternavn,
      unilogin,
      ...
      kontaktlaerer1: k1,
      kontaktlaerer2: k2,
      kontaktlaerer1_initialer: k1_ini,
      kontaktlaerer2_initialer: k2_ini
    };
  }
...
$('preview').textContent = buildStatement(st, getSettings());
  ...
  function renderMarksTable() {
    ...
    const kgrpLabel = (st) => {
      ...
      return groupKeyFromStudent(st);
    };

    function renderTickButton(unilogin, key, checked){
    ...
    function bindTickButtons(storeKey){
    ...
    if (type === 'sang') {
      ...
      `;
      bindTickButtons(KEYS.marksSang);
      return;
    }
